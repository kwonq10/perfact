...
function req({
  ...
  return new Request('https://example.com/api/quota/release', init);
}
...
function stubRpc(result = [{ ok: true, code: 'ok', state: 'released', used: 2 }]) {
  const calls = [];
...
test('GET は 405', async () => {
  const d = deps();
  const res = await handleRelease(req({ method: 'GET' }), ENV, d);
...
test('Origin なし → 403（session も RPC も呼ばない）', async () => {
  const d = deps();
  const res = await handleRelease(req({ origin: null }), ENV, d);
...
test('evil Origin → 403', async () => {
  for (const origin of ['https://evil.example', 'https://sukimacalendar.com.evil.example', 'null']) {
    const d = deps();
    const res = await handleRelease(req({ origin }), ENV, d);
...
test('正規 Origin のみ通る', async () => {
  const d = deps();
  assert.equal((await handleRelease(req(), ENV, d)).status, 200);
  assert.equal(d.rpc.calls.length, 1);
});

test('Content-Type が JSON でない → 400 invalid_content_type', async () => {
  const res = await handleRelease(req({ contentType: 'text/plain' }), ENV, deps());
...
test('malformed JSON → 400 malformed_json（RPC を呼ばない）', async () => {
  const d = deps();
  const res = await handleRelease(req({ body: '{"reservation_id":' }), ENV, d);
  assert.equal(res.status, 400);
...
test('1KB 超 → 413 body_too_large', async () => {
  ...
  const res = await handleRelease(req({ body }), ENV, deps());
...
test('reservation_id が UUID でない → 400 invalid_reservation_id（RPC を呼ばない）', async () => {
  for (const id of ['not-a-uuid', '', null, 123, undefined, 'c9f17da8e42646b9ac84b1b6159fc53c']) {
    const d = deps();
    const res = await handleRelease(req({ body: JSON.stringify({ reservation_id: id }) }), ENV, d);
...
test('body の user_id は無視され、session の user_id が使われる', async () => {
  ...
  await handleRelease(req({ body }), ENV, d);
...
test('未認証 → 401 unauthenticated + Cookie 削除', async () => {
  ...
  const res = await handleRelease(req({ cookie: null }), ENV, d);
...
test('session data_error / misconfigured / unavailable を潰さない', async () => {
  ...
  for (const [status, expected, error] of cases) {
    ...
    const res = await handleRelease(req(), ENV, d);
...
// =========================================================
// RPC の結果を透過する（状態機械の全分岐）
// =========================================================

test('ok → 200 でそのまま返す（released は used から外れる）', async () => {
  const d = deps();
  const res = await handleRelease(req(), ENV, d);
  assert.equal(d.rpc.calls[0].name, 'release_weekly_usage');
  assert.equal(res.status, 200);
  assert.deepEqual(await bodyOf(res), {
    quota_enforced: true, ok: true, code: 'ok', state: 'released', used: 2,
  });
});

test('released の冪等再送も ok', async () => {
  const d = deps({ rpc: stubRpc([{ ok: true, code: 'ok', state: 'released', used: 2 }]) });
  const body = await bodyOf(await handleRelease(req(), ENV, d));
  assert.equal(body.ok, true);
  assert.equal(body.state, 'released');
});

test('already_committed → 200 + ok=false', async () => {
  const d = deps({ rpc: stubRpc([{ ok: false, code: 'already_committed', state: 'committed', used: 3 }]) });
  const res = await handleRelease(req(), ENV, d);
  assert.equal(res.status, 200);
  assert.deepEqual(await bodyOf(res), {
    quota_enforced: true, ok: false, code: 'already_committed', state: 'committed', used: 3,
  });
});

test('release_budget_exceeded → 200 + ok=false / committed で確定', async () => {
  const d = deps({ rpc: stubRpc([{ ok: false, code: 'release_budget_exceeded', state: 'committed', used: 1 }]) });
  const res = await handleRelease(req(), ENV, d);
  assert.equal(res.status, 200, '予算切れも 4xx にしない');
  assert.deepEqual(await bodyOf(res), {
    quota_enforced: true, ok: false, code: 'release_budget_exceeded', state: 'committed', used: 1,
  });
});

test('expired → 200 + ok=false / committed で確定', async () => {
  const d = deps({ rpc: stubRpc([{ ok: false, code: 'expired', state: 'committed', used: 3 }]) });
  const res = await handleRelease(req(), ENV, d);
  assert.equal(res.status, 200);
  assert.deepEqual(await bodyOf(res), {
    quota_enforced: true, ok: false, code: 'expired', state: 'committed', used: 3,
  });
});
...
test('未知の code もそのまま透過する', async () => {
  ...
  assert.equal((await bodyOf(await handleRelease(req(), ENV, d))).code, 'some_future_code');
});
...
test('すべての応答に no-store と Vary: Cookie が付く', async () => {
  for (const r of [req({ method: 'GET' }), req({ origin: null }), req({ body: '{' }), req()]) {
    const res = await handleRelease(r, ENV, deps());
